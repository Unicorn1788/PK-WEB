...
import { getCachedDB, QueryDBResult } from "./QueryDB"
//import { formatUnits } from "ethers"

const RANK_BOOSTS: Record<number, number> = {
  0: 0.00,  // Recruit
  1: 0.05,  // Soldier
  2: 0.10,  // Knight
  3: 0.15,  // Duke
...
const RewardsOverview = () => {
  ...
  const [rankBoostRate, setRankBoostRate] = useState(0);
  const [queryDBResult, setQueryDBResult] = useState<QueryDBResult>({
    data: { allStakes: [] },
    loading: true,
    totalUnclaimed: 0,
    rewardsPerSecond: 0
  ...
  useEffect(() => {
    const fetchUserRank = async () => {
      ...
      try {
        ...
        const rankId = Number(result);
        const boostRate = RANK_BOOSTS[rankId] || 0;
        setRankBoostRate(boostRate);
      } catch (err) {
  ...
  // Update local state when address or cache changes
  useEffect(() => {
    if (!address) return;
  ...
  useEffect(() => {
    ...
    if (cached) {
      setQueryDBResult(cached);
      const { totalUnclaimed, rewardsPerSecond } = cached;

      // Filter active stakes and calculate the total remaining cap
      const activeStakes = allStakes.filter(stake => stake.active);
      const remainingCap = activeStakes.reduce((sum, stake) => sum + stake.remainingCap, 0);

      const totalUnclaimedMatic = totalUnclaimed / 1e18;
      const rewardsPerSecondMatic = rewardsPerSecond / 1e18;

      // Set live rewards based on the condition
      if (totalUnclaimedMatic > remainingCap) {
        setLiveRewards(remainingCap.toFixed(4)); // Set to remaining cap if unclaimed exceeds it
        return; // Exit early to prevent setting up the interval
      } else {
        setLiveRewards(totalUnclaimedMatic.toFixed(4)); // Otherwise, set to total unclaimed
      }

      const interval = setInterval(() => {
        setLiveRewards(prev => {
          const newValue = Number(prev) + rewardsPerSecondMatic;

          // Check against the remaining cap
          if (newValue >= remainingCap) {
            clearInterval(interval);
            return remainingCap.toFixed(4); // Set live rewards to the remaining cap
          }
  ...
  }, [isConnected, address, allStakes.length]);
  ...
  if (!queryDBResult || queryDBResult.loading) {
