...
// Utility: Get cached data for an address (with optional filter)
export function getCachedDB(address: string, options: QueryConditions = {}): QueryDBResult | null {
  const cacheKey = `${address}-stakes`;
  const cachedData = stakeCache.get(cacheKey);
  //console.log('[CACHE GET]', cacheKey, cachedData);
  if (cachedData && Date.now() - cachedData.timestamp < CACHE_DURATION) {
    const filteredStakes = options.filter
      ? cachedData.data.allStakes.filter(stake => {
          if (options.filter === "active") return stake.active;
          if (options.filter === "inactive") return !stake.active;
          return true;
        })
      : cachedData.data.allStakes;
    return {
      data: { allStakes: filteredStakes },
      totalUnclaimed: Number(cachedData.totalUnclaimed),
      rewardsPerSecond: Number(cachedData.rewardsPerSecond),
      loading: false,
    };
  }
  return null;
...
// Utility: Clear cache for an address
