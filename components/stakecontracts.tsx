...
const StakeContracts: React.FC<{ liveRewards: string }> = ({ liveRewards }) => {
  ...
  // Convert totalUnclaimed from gwei/wei to MATIC
  const totalUnclaimedMatic = typeof totalUnclaimed === 'number' ? totalUnclaimed / 1e18 : 0;

  // Filter active stakes
  const activeStakes = allStakes.filter((stake: StakeData) => stake.active);
          ...
          >
            ...
            <div className="bg-[#18102a]/80 rounded-xl p-4 border border-[#a58af8]/10 flex flex-col items-center mb-4">
              <div className="flex gap-4 mb-2">
                ...
                <span className="text-xs text-white/60">Total Claimable: <span className="font-bold text-[#facc15]">{formatNumber(totalUnclaimedMatic)} POL</span></span>
              </div>
            </div>
            {loading ? (
              ...
              filteredStakes.map((stake: StakeData, index: number) => {
                // Set claimable amount to live rewards
                const claimableAmount = parseFloat(liveRewards);
                      ...
                      {expandedCards[index] && (
                        ...
                        >
                          <div className="bg-[#18102a]/80 rounded-xl p-4 border border-[#a58af8]/10 flex flex-col items-start">
                            ...
                            {/* <span className="text-xs text-white/60 mb-1">Claimable: {formatNumber(claimableAmount)} POL</span> */}
